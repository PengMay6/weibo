...
def parse_json(response, articleDataFilePath, type='无'):
    for article in response:
        ...
        # print(commentsLen)
        if int(commentsLen) < 20: continue
        id = article['id']
...
def start(types=[], page=0, articleCategoryFilePath='', articleDataFilePath=''):
    ...
    for type in typeList:
        ...
        for currentPage in range(0, page):
            ...
            # 使用 urlencode 来构建查询参数
            query_string = urllib.parse.urlencode(parmas)
            # 拼接最终的 URL
            full_url = f"{articleUrl}?{query_string}"
            print(full_url)
...
def start_2(url, articleDataFilePath, type):
    print(articleDataFilePath,123)
    init(articleDataFilePath)
    mblogid = getId(url)
    if mblogid is None: return False
    response = requests.get('https://weibo.com/ajax/statuses/show?id=' + mblogid, headers=headers2)
    ...
    if content.find("...<span class=\"expand\">展开</span>") != -1:
        response_longTextContent = requests.get('https://weibo.com/ajax/statuses/longtext', headers=headers2,
                                                params={'id': mblogid})
        if response_longTextContent.status_code == 200:
            content = response_longTextContent.json()['data']['longTextContent']
    else:
