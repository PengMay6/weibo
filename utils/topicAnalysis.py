...
from globalVariable import headers2


def getCiTiaoList():
    response = requests.get('https://weibo.com/ajax/side/hotSearch', headers=headers2)
    text = response.json()
...
def getWeiboAI(topicName):
    response = requests.get('https://ai.s.weibo.com/api/llm/analysis_tab.json?query=' + topicName, headers=headers2)
